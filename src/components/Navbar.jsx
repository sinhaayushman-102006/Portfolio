      ...
      <nav className={`fixed top-0 left-0 w-full z-50 flex items-center justify-between px-6 py-5 md:px-12 md:py-6 transition-all duration-700 ease-in-out ${show ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0'} ${isMenuOpen ? 'bg-black/90 backdrop-blur-md' : 'bg-transparent'}`}>
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <a
            ...
            className="px-4 py-1.5 rounded-full border border-[#ccff00] text-[#ccff00] hover:bg-[#ccff00] hover:text-black transition-all text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 cursor-pointer shadow-[0_0_15px_rgba(204,255,0,0.15)] hover:shadow-[0_0_20px_rgba(204,255,0,0.4)]"
          >
            <span>Resume</span>
      ...
      <div className={`fixed inset-0 z-40 bg-[#0a0a0a] flex flex-col items-center justify-center transition-all duration-500 ease-in-out md:hidden ${isMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
        <div className="flex flex-col items-center gap-8">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className={`text-[#ccff00] text-xl font-bold uppercase tracking-widest border border-[#ccff00] px-6 py-2.5 rounded-full hover:bg-[#ccff00] hover:text-black transition-all flex items-center gap-2 mt-2 ${isMenuOpen ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}
            onClick={() => setIsMenuOpen(false)}
          >
